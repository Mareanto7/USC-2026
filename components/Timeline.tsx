...
function EditionText({ ed }: {ed: any }) {
    return (
        <div className="text-white">
            <h2 className="text-4xl font-bold mb-6">{ed.year}</h2>
...
export default function Timeline() {
    return (
        <div className="bg-[#252a5a]">
            <h2 className="pt-25 text-center text-5xl font-bold tracking-widest text-white">Our past editions</h2>

            <div className="mt-25 relative">
                {/* --- CENTER LINE --- */}
                <div className="absolute left-1/2 top-[35vh] bottom-0 w-0.5 bg-white -translate-x-1/2"/>
                {CONTENT.about.editions.map((ed, idx) => (
                    <div key={idx}
                         className={`flex items-center w-full min-h-[70vh] ${idx % 2 === 0 ? 'bg-[#252a5a]' : 'bg-[#e74528]'}`}>

                        {/* --- LEFT SIDE --- */}
                        <div className="w-1/2 px-16 flex justify-end">
                            {idx % 2 === 0 ? <img src={ed.image} className="w-full max-w-md rounded-xl"/> :
                                <EditionText ed={ed}/>}
                        </div>


                        {/* --- DOT --- */}
                        <div className="w-10 h-10 rounded-full bg-white flex-shrink-0 z-10"/>

                        {/* --- RIGHT SIDE --- */}
                        <div className="w-1/2 px-16">
                            {idx % 2 === 0 ? <EditionText ed={ed}/> :
                                <img src={ed.image} className="w-full max-w-md rounded-xl"/>}
                        </div>
                    </div>
                ))}
            </div>
